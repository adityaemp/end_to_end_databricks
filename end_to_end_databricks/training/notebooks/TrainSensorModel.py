...
dbutils.widgets.text('model_name', 'dev.sensor_streaming.sensor_model')

dbutils.widgets.text('gold_feature_table', 'sensor_streaming.gold_sensor_features')
...
gold_df = spark.table(gold_feature_table).na.drop()
...
with mlflow.start_run() as run:
    ...
    if output_stage in ['Staging', 'Production']:
        ...
        latest = client.get_latest_versions(model_name, [output_stage])
        # transition the new version to requested stage
        new_version = client.get_model_version_by_label(model_name, 'None').version if latest else run.info.run_id
