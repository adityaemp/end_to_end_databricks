...
|----------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ML Code                    | Example ML project code, with unit tested Python modules and notebooks; extended with sensor streaming source, Delta Live Tables medallion ETL, and MLflow training.                                                                                                                                                                                                                               |
| ML Resources as Code | ML pipeline resources (training and batch inference jobs with schedules, etc) configured and deployed through [databricks CLI bundles](https://docs.databricks.com/dev-tools/cli/bundle-cli.html); includes `sensor-streaming-workflow-resource.yml` and `dlt-sensor-streaming-pipeline.json` for bronze/silver/gold DLT pipelines.                                                                                              |
...
```
end_to_end_databricks        <- Root directory. Both monorepo and polyrepo are supported.
│
├── end_to_end_databricks       <- Contains python code, notebooks and ML resources related to one ML project. 
│   │
...
the [project README](./end_to_end_databricks/README.md).
...
After that, follow the [ML pull request guide](docs/ml-pull-request.md)
 and [ML resource config guide](end_to_end_databricks/resources/README.md)  to propose, test, and deploy changes to production ML code (e.g. update model parameters)
or pipeline resources (e.g. use a larger instance type for model training) via pull request.

| Role                          | Goal                                                                         | Docs                                                                                                                                                                |
|-------------------------------|------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Data Scientist                | Get started writing ML code for a brand new project                          | [project README](./end_to_end_databricks/README.md) |
...
| Data Scientist                | Modify production model ML resources, e.g. model training or inference jobs  | [ML resource config guide](end_to_end_databricks/resources/README.md)  |
...
need to run it again for project `end_to_end_databricks`.
